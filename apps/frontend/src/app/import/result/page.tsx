...
function ResultContent() {
  ...
  if (error || !job) {
    return (
      <div className="text-center p-12 space-y-4 max-w-md mx-auto">
        <h2 className="text-xl font-bold text-red-650">Failed to load results</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={() => router.push('/')}
          className="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold transition-all active:scale-98"
        >
        ...
        {job.failedImports.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden divide-y divide-slate-200 dark:divide-slate-800">
              {job.failedImports.slice(0, 5).map((failed) => (
