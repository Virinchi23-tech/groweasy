...
export default function PreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: previewData, isLoading, error } = useQuery<PreviewResponse>({
    ...
    queryFn: () => apiFetch(`/csv/status/${id}`), // The endpoint GET /csv/status/:id returns preview data on initial load
  });
  ...
  if (error || !previewData) {
    return (
      <div className="text-center p-12 space-y-4 max-w-md mx-auto">
        <h2 className="text-xl font-bold text-red-600">Failed to load preview</h2>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 p-4 rounded-3xl shadow-sm">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-slate-900 dark:text-white"
          />
        </div>
      </div>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-8 border border-slate-200 dark:border-slate-800 shadow-2xl animate-scaleUp space-y-6">
            ...
            {/* Estimates box */}
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl p-4 divide-y divide-slate-250 dark:divide-slate-800 space-y-3">
