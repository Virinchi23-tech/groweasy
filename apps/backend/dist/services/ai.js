...
exports.mapRecordsWithAI = void 0;
...
const mapHeuristically = (records) => {
    logger_1.aiLogger.info('Using heuristic rule-based mapping fallback.');
    return records.map((record) => {
        ...
        if (statusVal) {
            const matchingStatus = shared_1.CRM_STATUSES.find(s => s.toLowerCase() === statusVal.toLowerCase() || statusVal.toLowerCase().includes(s.toLowerCase()));
            if (matchingStatus)
                crm_status = matchingStatus;
        }
        // Guess data source
        let data_source = null;
        const sourceVal = getVal(/source/i);
        if (sourceVal) {
            const matchingSource = shared_1.DATA_SOURCES.find(s => s.toLowerCase() === sourceVal.toLowerCase() || sourceVal.toLowerCase().includes(s.toLowerCase()));
            if (matchingSource)
                data_source = matchingSource;
        }
        return {
...
const mapRecordsWithAI = async (records, headers) => {
    ...
    try {
        logger_1.aiLogger.info(`Sending batch of ${records.length} records to OpenAI.`);
...
Records batch: ${JSON.stringify(records)}`;
        ...
        if (parsed.success && Array.isArray(parsed.records)) {
            logger_1.aiLogger.info(`Successfully mapped batch of ${parsed.records.length} records.`);
        ...
        logger_1.aiLogger.error('AI column mapping error', { error: err.message });
        return { success: false, records: [], error: err.message };
    }
};
exports.mapRecordsWithAI = mapRecordsWithAI;
